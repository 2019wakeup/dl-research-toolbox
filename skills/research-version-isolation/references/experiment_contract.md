...
Use the bundled guard to enforce this contract:

```bash
python3 <skill-dir>/scripts/research_memory_guard.py --root . --rebuild-experiment-index
bash <skill-dir>/scripts/install_research_hooks.sh .
```

The guard rejects flat run directories directly under `research/experiments/`, missing required files, invalid phase/series/run slugs, and run directories missing from `index.jsonl`.
